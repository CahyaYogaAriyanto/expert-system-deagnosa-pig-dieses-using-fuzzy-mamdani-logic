...
def login_cek():
    ...
    if result.data:
        ...
        if user['password'] == password:
            ...
            session['user_id'] = user.get('id_user')
...
@app.route('/lc_pakar')
def lc_pakar():
    return render_template("coba.html",username=session["pakar_nama"])
...
@socketio.on("join")
def on_join(data):
    user_id = session["user_id"]  # ambil dari frontend
    if not user_id:
        print("[ERROR] join tanpa user_id:", data)
        return
    room = str(user_id)
    join_room(room)
    print(f"[JOIN] {user_id} masuk ke room {room}")
...
def chat_history():
    pakar = request.args.get("pakar_id")
